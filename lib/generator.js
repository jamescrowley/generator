  ...
  installTemplate(force = false) {
    return new Promise(async (resolve, reject) => {
      ...
      if (isFileSystemPath(this.templateName)) {
        templatePath = path.resolve(this.templateName);
      } else { // Template is not a filesystem path...
        templatePath = path.resolve(DEFAULT_TEMPLATES_DIR, this.templateName);
        const templateExistsInCache = await exists(templatePath);
        if (!templateExistsInCache || force) {
          npmi({
        ...
        // follow symlinks
        if (templateExistsInCache && await isLocalTemplate(templatePath)) {
          ...
          templatePath = resolvedLink;
        }
      }

      if (!exists(templatePath))
        throw new Error(`The template ${templatePath} does not exist.`);

      const packageJsonPath = path.resolve(templatePath, PACKAGE_JSON_FILENAME);
      if (!exists(packageJsonPath))
        throw new Error(`This template found at ${templatePath} does not have a package.json, and one is required.`);

      const installedPkg = require(packageJsonPath);
      return resolve({
